...
void TerrainTile::Init(pTerrain inTerrain, int2 inIndex, pMaterial inMaterial, pMaterial inShadowMaterial)
{
	...
	for (int y = 0; y < resolution.y; y++)
	{
		...
		for (int x = 0; x < resolution.x; x++)
		{
			...
			vert.Position = float3((u - 0.5) * scale.x, (v - 0.5) * scale.y, 0);
	...
	float3 tile_pos;
	tile_pos.x = (float(inIndex.x) - float(inTerrain->GetNumTiles().x - 1) / 2.0f) * scale.x;
	tile_pos.y = (float(inIndex.y) - float(inTerrain->GetNumTiles().y - 1) / 2.0f) * scale.y;
	tile_pos.z = 0.0f;
	Translate(tile_pos);

	mConstantBuffer = theResourceFactory.MakeConstantBuffer(sizeof(mConstantBufferData));
	mConstantBufferData.scale = float4(scale, 0.0f);
